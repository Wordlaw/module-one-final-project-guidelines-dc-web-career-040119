class Candidate < ActiveRecord::Base
  ...
  def self.list_candidates(eligible_candidate_arr)
    puts " "
    puts "These are you candidates based on eligible years of experience".yellow
  ...
  def self.select_candidate(eligible_candidate_arr, company_choice_index, job_choice_index)
    ...
    if eligible_candidate_arr[candidate_choice_index][:salary_expectations] < job_salary
      puts " "
      puts "Congratulations!!! You hired #{eligible_candidate_arr[candidate_choice_index][:name]}".yellow
    else
      puts " "
      puts "We're sorry #{eligible_candidate_arr[candidate_choice_index][:name]}'s salary expectations exceed the job requistion ".red
    end
